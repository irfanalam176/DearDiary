...
const Protected = ({ navigation, route }) => {
  ...
  async function getNotes() {
    try {
      const response = await DiaryRepository.getProtectedEntries();
  ...
  async function deleteNote(id) {
    try {
      const response = await DiaryRepository.deleteProtectedEntry(id);
      if (response.rowsAffected > 0) {
        getNotes();
      }
      await useDelete(id,"protected")
    } catch (e) {
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View style={{ paddingHorizontal: 25 }}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('writeProtected', { category: category })
            }
          >
          ...
          {filteredNotes.map((item, idx) => (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                navigation.navigate('viewProtected', {
                  id: item.id,
