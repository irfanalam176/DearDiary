...
const ViewProtected = ({ navigation, route }) => {
  ...
  // ✅ Handle update with interstitial
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await DiaryRepository.updateProtectedEntry(entry);
      if (response.rowsAffected > 0) {
        await useUpdateSync(id, title, htmlContent, formatDate(date), 'protected');
  ...
  const htmlTemplate = `
    ...
    <body>
      <div id="editor" class="lined-textarea" contenteditable="true" placeholder="Dear Diary...">${content}</div>
      <script>
      ...
      <View style={{ padding: 16 }}>
        <View style={style.noteHeader}>
          <View style={style.header}>
            ...
            <Text style={style.headerText}>DearDiary - Protected</Text>
          </View>
