...
const Notes = ({ navigation, route }) => {
  ...
  async function getNotes() {
    try {
      const response = await DiaryRepository.getEntries();
  ...
  async function deleteNote(id) {
    try {
      const response = await DiaryRepository.deleteEntry(id);
      if (response.rowsAffected > 0) {
        getNotes();
      }
      await useDelete(id,"entries")
    } catch (e) {
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View style={{ paddingHorizontal: 25 }}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('writeNote', { category: category })
            }
          >
