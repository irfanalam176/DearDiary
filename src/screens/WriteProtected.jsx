...
const WriteProtected = ({ navigation, route }) => {
  const { category } = route.params;

  const [title, setTitle] = useState("");
  const [htmlContent, setHtmlContent] = useState('');
  const webViewRef = useRef(null);
  const [webViewHeight, setWebViewHeight] = useState(Dimensions.get('window').height * 0.6);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await DiaryRepository.createProtectedEntry(entry);
      await useSync();
  ...
  const htmlTemplate = `
    ...
    <body>
      <div id="editor" class="lined-textarea" contenteditable="true" placeholder="Dear Diary..."></div>
      <script>
